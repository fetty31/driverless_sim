...
Accumulator::Accumulator(physics::ModelPtr &_parent,
            sdf::ElementPtr &_sdf,
    ...
    double mean, stdd;
    nh->param<double>("/Tracker/Noise/Mean", mean, 0.0);
    nh->param<double>("/Tracker/Noise/Std",  stdd, 0.0);

    // Declare noise ptr obj
    noise = new noise::Gaussian(mean, stdd);

    nh->param<double>("/Tracker/Detection/Radius",      this->radius,       20.0);
    nh->param<double>("/Tracker/Detection/VisionAngle", this->vision_angle, 180.0);
    nh->param<double>("/Tracker/Likelihood",            this->likelihood,   0.9);
    nh->param<double>("/Tracker/BoundingBox/Lf",        this->Lf,           1.5);
    nh->param<double>("/Tracker/BoundingBox/Lr",        this->Lr,           1.0);
    nh->param<double>("/Tracker/BoundingBox/T",         this->T,            1.42);

    nh->param<bool>("/Tracker/ConesMoving", this->moveConesFlag, true);
...
void Accumulator::fillConeMsg(int id, dv_common::Cone* msg){
    msg->id = track[id].id;
...
dv_common::LapInfo Accumulator::getLapInfo(){
    dv_common::LapInfo msg;
...
bool Accumulator::isInLidarField(int id, StateCar* state){
    ...
    if(phi < -M_PI) phi += 2*M_PI; // convert phi into [-180, 180] domain
...
// Get all seen cones
void Accumulator::getSeenCones(StateCar* state, dv_common::ConeArray* coneArray){

    dv_common::Cone coneMsg = dv_common::Cone();
